...
| 4 | `power`      | `build_power.py`              | score track: 3 tokens/player, score = SUM of ring positions; 4 staggered advances (orange 4→5, purple 4→5, green 3→4, purple 0→1) → green 4 / orange 5 / purple 6; orange & purple flank the gold apex (one each side), every token sits OFF its ring's value-node so the printed dots show, idle reserves piled in per-player coin-stacks on the base; each move carries a player-tinted plasma bell |
