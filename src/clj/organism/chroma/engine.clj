(ns organism.chroma.engine
  "Pure Chroma game engine — a faithful Clojure port of game-ideas/chroma/chroma-core.js.
...
(def palettes
  {:RYB {:order ["R" "O" "Y" "G" "B" "P"]
         ;; Recalibrated 2026-06-20 from the flower-suncatcher photo + artist-wheel theory
         ;; so the three NEIGHBOUR mixes land on their secondaries: R+B->Purple, Y+B->Green
         ;; (was mud — the reported bug), R+Y->Orange (was Red). Key levers: purer Blue
         ;; (red-leak 0.30->0.18) and purer Yellow (blue-leak 0.32->0.19) make B*Y a clean
         ;; green inside PURITY_MAX. PROVISIONAL — final numbers come from the gel calibration
         ;; shoot; casual photo only set the direction. See game-ideas/chroma/chroma-photos/
         ;; 2026-06-20_RYB-recalibration_PROVISIONAL.json for the full current->new cell diff.
         :trans {"R" [0.95 0.35 0.31] "O" [0.98 0.52 0.19] "Y" [0.94 0.94 0.19]
                 "G" [0.23 0.80 0.49] "B" [0.18 0.41 0.93] "P" [0.65 0.38 0.86]}
...
;; CANONICAL 73-cell board (Mohammad 2026-07-02, mirrors chroma-core.js DEFAULT_TRIM):
;; the 9-cell corner notches (2026-06-14) PLUS the 3-cell inner bulge behind each —
;; each white corner's 7-in-a-line becomes two discontinuous rim pairs. 120°-symmetric;
;; whites 42 -> 24, wedges alternate 9/15 playable. Older saved games keep the trim
;; they were created with (:removed is per-game state).
(def trim-cells
  #{[5 0] [4 1] [5 -1] [-5 5] [-5 4] [-4 5] [0 -5] [1 -5] [-1 -4]          ; corner notches
    [-4 3] [-4 4] [-3 4] [-1 -3] [0 -4] [1 -4] [3 1] [4 0] [4 -1]})        ; inner bulges
...
;; ── visibility pools (Mohammad 2026-07-02, mirrors chroma-core.js) ───────────
;; Starting hand is HIDDEN; every later gain (draw/swap) is PUBLIC, tracked per
;; color in (:pub p). Invariant: pub[c] <= count of c in hand. Spending a color
;; held in both pools is a choice (:fromHidden on the move/plan); the default
;; spends the public copy first, preserving the hidden "shadow".
(defn hand-count [p color] (count (filter #(= % color) (:hand p))))
(defn pub-count [p color] (min (get-in p [:pub color] 0) (hand-count p color)))
(defn hidden-count [p color] (- (hand-count p color) (pub-count p color)))
(defn- spend-chit
  "Remove one `color` chit from pi's hand honoring the pool choice; clamps pub<=hand."
  [G pi color from-hidden?]
  (let [hand (get-in G [:players pi :hand])
        i (.indexOf ^clojure.lang.PersistentVector hand color)]
    (if (neg? i)
      G
      (let [hand' (vec (concat (subvec hand 0 i) (subvec hand (inc i))))
            cnt (count (filter #(= % color) hand'))
            pc (get-in G [:players pi :pub color] 0)
            pc' (cond
                  (and from-hidden? (pos? (- (inc cnt) pc))) pc   ; a hidden copy was spent
                  (pos? pc) (dec pc)
                  :else pc)]
        (-> G
            (assoc-in [:players pi :hand] hand')
            (assoc-in [:players pi :pub color] (min pc' cnt)))))))
(defn- gain-public [G pi color] (update-in G [:players pi :pub color] (fnil inc 0)))

(defn apply-placement
  "THE single mutator. Returns [G' placement-record]. move may carry :fromHidden."
  [G pi move]
  (let [c (:c move) color (:chit move) k c
        st (get-in G [:stacks k] [])
        prev (if (seq st) (classify (mix-stack st)) "white")
        p (get-in G [:players pi])
        G' (-> (spend-chit G pi color (boolean (:fromHidden move)))
    ...
    [G' {:pi pi :k k :c c :color color :fromHidden (boolean (:fromHidden move))
         :prev prev :res (:color rr)
             ...
             (if (>= avail (count claim))
               [(reduce (fn [g pp]
                          (-> g (update-in [:bag c] dec)
                              ...
                              (gain-public (:pi pp) c)))   ; visibility: draws are public
                        G claim)
    ...
    (reduce
     (fn [[G granted denied] [c claim]]
       (if (>= (get-in G [:bag c] 0) (count claim))
         (let [G' (reduce
                   (fn [g s]
                     (let [g (reduce (fn [gg d] (spend-chit gg (:pi s) d (boolean (:fromHidden s))))
                                     g (:discards s))]   ; visibility: pool-aware discards
                       (-> g
                           (update-in [:players (:pi s) :hand] (fnil conj []) c)
                           (gain-public (:pi s) c)        ; visibility: swap gains are public
                           (update-in [:bag c] dec)
  ...
  (binding [*palette-key* palette *pal* (palettes palette) *depth* depth *removed* removed]
    (let [rng (make-rng seed)
          ...
          players0 (mapv (fn [spec i]
                           {:g (or (:g spec) (default-genome)) :hand [] :pub {} :dry 0 :swaps 0 :discarded 0
