(ns organism.routes.universal-ws
  "WebSocket handler for universal game play.
   Same pattern as journey-ws but uses organism.universal.game."
  (:require
   [clojure.edn :as edn]
...
(defn ai-pick-action
  "Simple heuristic agent: prefer capture > eat > grow > move > circulate > pass."
  [actions]
  (let [by-type (group-by (comp first first) actions)
        pick    (fn [t] (when-let [acts (get by-type t)] (rand-nth acts)))]
    (or (pick :capture)
        (pick :eat)
        (pick :grow)
        (pick :move)
        (pick :circulate)
        ;; fallback: random non-pass action, or pass
            ...
            (let [actions (compute-legal-actions rs bd st)]
              (when (seq actions)
                (let [[ak ns] (ai-pick-action (vec actions))]
                  ...
                  (log/info "AI move" play-key (:current-player st) (pr-str ak))
                  (Thread/sleep 300)
  ...
  (let [choice (or (get message "choice") (get message :choice))
    ...
    (when (and choice state ruleset)
      (try
        ...
        (catch Exception e
          (log/error "Failed to apply universal action" play-key player-key
                     choice (.getMessage e)))))))
...
(defn notify-clients! [{:keys [play-key player]} _channel raw]
  (let [message (read-json raw)
    ...
    (case msg-type
      ...
      (log/warn "Unknown universal message type" msg-type))))
...
(defn universal-ws-routes []
  [["/ws/universal/play/:play" ws-handler]])
