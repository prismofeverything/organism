  ...
  (when (and you (not (:over G)))
    (let [p (get-in G [:players you])
      ...
      (cond-> {:seat you :hand (:hand p)
               :pub (or (:pub p) {})     ; visibility rule: which of MY chits are open
         ...
         :seats (mapv (fn [i]
                        (let [p (nth (:players G) i)]
                          {:seat i :name (:name p) :isBot (boolean (contains? (:bots server) i))
                           ...
                           ;; visibility rule: everyone sees each seat's PUBLIC (drawn) chits;
                           ;; hidden = handCount - sum(pub)
                           :pub (or (:pub p) {})
...
(defn handle-place! [db play-key player {:keys [c chit pass fromHidden]}]
  (let [server (get-in @games [:games play-key])
