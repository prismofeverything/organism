(ns organism.oroboros.play
  "Universal game frontend: renders any board game and handles interaction."
  (:require
   ...
   [organism.oroboros.ruleset :as rs]
   [organism.oroboros.topology :as topo]
   [organism.oroboros.describe :as desc]))
...
(defn- v3-elements
  "Extract elements from state in a format compatible with the renderer.
...
(defn- v3-free-food
  "Extract free resources from state (resources on unoccupied nodes)."
  [state]
  (when-let [nodes (:nodes state)]
    (reduce-kv
     (fn [m node nd]
...
(defn- get-current-player [state _ruleset]
...
(defn compute-locations
  ...
  [topology-type coords grid-size adjacencies]
  (when (and coords (seq coords))
    ;; Coord-based rendering (all topologies including radial)
...
(defn handle-click! [space]
  (let [state @game-state
    ...
    (js/console.log "click:" space "sel:" sel "actions:" (count actions)
                    "from-spaces:" (pr-str (set (map (comp second first) (vec actions)))))
    ...
    (cond
      ...
      (let [indexed (index-actions actions)
        ...
        (cond
          ...
          :else
          (let [elements (v3-elements state)
                el (get elements space)]
...
(defn game-board []
  (let [state @game-state
    ...
    (when (and state ruleset board)
      (let [adjacencies (:adjacencies board)
            topology-type (get-in ruleset [:schema :topology_type] "hex")
            coords (:coords board)
            grid-size (:grid-size board)
            locations (compute-locations topology-type coords grid-size adjacencies)
            current-p (str (get-in state [:globals "current_player"] 0))
        ...
        [:svg {:width "100%" :height "100%"
         ...
         ;; Free food
         (let [free-food (v3-free-food state)]
...
(defn conflict-explanation
  ...
  [ruleset]
  (let [n (get-in ruleset [:schema :num_piece_types])
        conflict (vec (:conflict_table ruleset))
        type-name (fn [t] (str "Type " t))]
    (when (and n (> n 1) (seq conflict))
      [:div {:style {:margin-top "8px"}}
       ...
       (for [i (range n)
             j (range (inc i) n)
             :let [idx (loop [x 0 k 0]
                         (if (= k i) (+ x (- j i 1))
                           (recur (+ x (- n 1 k)) (inc k))))
                   raw (get conflict idx 0)
                   outcome (case (int raw)
                             0 :coexist
                             3 :mutual
                             (let [lo (min i j)
                                   winner (if (= raw 1) lo (max i j))]
                               (if (= winner i) :wins :loses)))]]
         ^{:key (str i "-" j)}
      ...
      (let [current (when state (get-current-player state ruleset))
        ...
        [:div {:style {:width "240px" :background "#0a0a14"
         ...
         (when state
           [:div {:style {:margin-bottom "10px" :padding "6px 8px" :border-radius "4px"
            ...
            [:div {:style {:color (if my-turn? "#66ff66" "#888")}}
             (str "Round " (get-in state [:globals "round"] 0) " — "
         ...
         (when state
           [:div {:style {:margin-bottom "10px"}}
            [:div {:style {:color "#666" :font-size "10px" :margin-bottom "4px"
                           :text-transform "uppercase" :letter-spacing "1px"}}
             "Players"]
            (let [players (mapv str (range (get-in ruleset [:schema :num_players] 2)))]
              (for [p players]
                (let [is-me? (= (str p) @player-key)
                      is-current? (= (str p) (str current))
                      elements (v3-elements state)
                      pop (count (filter #(= (str p) (str (:player %))) (vals elements)))
                      caps (get-in state [:globals (str "counter_" p)] 0)]
                ^{:key p}
         ...
         [:div {:style {:border-top "1px solid #222" :padding-top "8px" :margin-top "4px"}}
          ...
          [:div {:style {:margin-bottom "6px" :color "#aaa" :font-size "11px"}}
           (str (get-in ruleset [:schema :num_piece_types]) " piece types, "
                (get-in ruleset [:schema :num_players] 2) " players")]

          [:div {:style {:margin-bottom "6px" :color "#aaa" :font-size "11px"}}
           (let [tl (:turn_limit ruleset)]
             (str "Win by: turn limit"
                  (when tl (str " (limit: " tl " rounds)"))))]

          ;; Actions per turn
          (let [apt (or (:actions_per_turn ruleset) (:actions-per-turn ruleset) 1)]
            (when (> apt 1)
              [:div {:style {:margin-bottom "6px" :color "#aaa" :font-size "11px"}}
               (str "Turns: " apt " actions per turn")]))
          ...
          [:div {:style {:margin-top "8px"}}
           [:div {:style {:color "#666" :font-size "10px" :margin-bottom "4px"
                          :text-transform "uppercase" :letter-spacing "1px"}}
            "Rules"]
           (for [[idx rule] (map-indexed vector (:rules ruleset))]
             ^{:key idx}
             [:div {:style {:font-size "11px" :padding "2px 0" :color "#999"}}
              (str "Rule " idx ": "
                   (or (desc/effect->text (:effect rule)) "action"))])]

          ;; Conflict table
          [conflict-explanation ruleset]

          ;; Resource info
          (let [num-res (get-in ruleset [:schema :num_resource_slots] 0)]
            (when (pos? num-res)
              [:div {:style {:margin-top "8px" :color "#999" :font-size "11px"}}
               (str num-res " resource slot(s)")]))]
...
(defn format-action
  ...
  (let [fmt-sp (fn [sp] (cond (nil? sp) "—"
                               ...
                               :else (str sp)))]
    (cond
      ;; Action-type is an integer (rule slot index) or "pass"
      (number? action-type)
...
(defn- deep-keywordize
  "Recursively convert string keys to keywords in maps."
  [m]
  (cond
    (map? m)    (into {} (map (fn [[k v]] [(if (string? k) (keyword k) k) (deep-keywordize v)]) m))
    (vector? m) (mapv deep-keywordize m)
    (seq? m)    (map deep-keywordize m)
    :else       m))

(defonce library-games
  (r/atom
   (let [lib-json (when (exists? js/gameLibraryJson) js/gameLibraryJson)]
     (if (and lib-json (array? lib-json))
       (mapv (fn [g]
               (let [rs (js->clj g :keywordize-keys true)
                     ruleset (:ruleset rs)
                     schema (:schema ruleset)]
                 {:name (str (:topology_type schema "?") " "
                             (:topology_size schema "")
                             " t" (:num_piece_types schema "?")
                             " r" (count (:rules ruleset)))
                  :richness (:richness rs 0)
                  :description (:summary rs "")
                  :ruleset ruleset}))
             lib-json)
       []))))
...
(defn create-view []
  (let [rs @create-ruleset
    ...
    [:div {:style {:color "#ccc" :font-family "monospace" :padding "30px 40px"
     ...
     [:h1 {:style {:color "#fff" :margin-bottom "4px"}} "Oroboros"]
     [:div {:style {:color "#666" :font-size "12px" :margin-bottom "20px"}}
      "Games discovered by evolutionary search through game space"]

     ;; Library games (from search results, sorted by richness)
     (when (seq lib)
       [:div {:style {:margin-bottom "16px"}}
        [:div {:style {:color "#ffaa44" :font-size "10px" :margin-bottom "8px"
                       :text-transform "uppercase" :letter-spacing "1.5px"}}
         (str (count lib) " discovered games (sorted by richness)")]
        (for [[idx {:keys [ruleset] game-name :name :as game}]
              (map-indexed vector (take 20 lib))]
          ^{:key (str "lib-" idx)}
     ...
     [:div {:style {:margin-bottom "16px"}}
      [:div {:style {:color "#cc88ff" :font-size "10px" :margin-bottom "8px"
                     :text-transform "uppercase" :letter-spacing "1.5px"}}
       (str (count rs/discovered-games) " canonical games")]
      (for [{:keys [ruleset] game-name :name :as game} rs/discovered-games]
        ^{:key (str "preset-" game-name)}
        [game-card game (= ruleset rs)])]
     ...
     ;; Play button
     [:a {:href (str "/oroboros/play/" game-id)
          :on-click (fn [_]
                      ;; Store selected ruleset so play page can use it
                      (.setItem js/localStorage "oroboros-ruleset" (pr-str rs)))
...
(defn- process-state-update! [state action-str]
  (let [me @player-key
    ...
    (when (and prev-state acted-player
      ...
      (swap! history-atom conj
             {:step (count @history-atom)
              :player acted-player
              :round (get-in prev-state [:globals "round"] 0)
    ...
    (js/console.log "state update: my-turn?=" my-turn? "current=" current "me=" me
                    ...
                    "globals=" (pr-str (:globals state)))
    (when @ruleset-atom
      (js/console.log "ruleset keys:" (pr-str (keys @ruleset-atom))
                      "schema keys:" (pr-str (keys (:schema @ruleset-atom)))
                      "rules:" (count (:rules @ruleset-atom))))
    (if (and my-turn? @ruleset-atom @board-atom (not (:winner state)))
      (try
        (let [actions (compute-legal-actions @ruleset-atom @board-atom state)]
          (js/console.log "legal actions:" (count actions)
                          "sample keys:" (pr-str (take 3 (keys actions))))
      ...
      (do
        (js/console.log "skipping legal actions: my-turn?=" my-turn?
                        "ruleset?=" (boolean @ruleset-atom)
                        "board?=" (boolean @board-atom)
                        "winner=" (:winner state))
        (reset! legal-actions-atom {})))))
    ...
    (case msg-type
      ...
      (do
        (when-let [rs-str (get message "ruleset")]
          (let [rs (deep-keywordize (reader/read-string rs-str))]
            (js/console.log "init ruleset keys:" (pr-str (keys rs)))
...
(defn rules-catalog-view []
  [:div {:style {:color "#ccc" :font-family "monospace" :padding "30px 40px"
   ...
   [:div {:style {:margin-bottom "20px" :display "flex" :gap "8px"}}
    [:a {:href "/oroboros" :style {:color "#4488ff" :font-size "12px"}} "< back to games"]]
  ...
  (let [play-key (.-playKey js/window)
    ...
    (cond
      ...
      (do (let [host (.-host js/location)
              ...
              url (str protocol "://" host "/ws/oroboros/play/" play-key)]
        ...
        (js/setTimeout
         (fn []
           (when-not @game-state
             (let [stored (.getItem js/localStorage "oroboros-ruleset")
