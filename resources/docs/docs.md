...
The `home-routes` handler in the `organism.routes.home` namespace
...
The routes are aggregated and wrapped with middleware in the `organism.handler` namespace:
...
Request middleware functions are located under the `organism.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `organism.dev-middleware` namespace found in
