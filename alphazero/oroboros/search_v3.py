"""Search v3: evolutionary search over rule-program games."""
...
def _dominates(a: SearchResultV3, b: SearchResultV3) -> bool:
...
class SearchV3:
    def __init__(
        ...
        checkpoint_dir: str        = "search_v3_checkpoints",
        verbose:        bool       = True,
    ):
        ...
        self.archive:     list[SearchResultV3] = []
        self.all_results: list[SearchResultV3] = []
        self.generation = 0

    def run(self, n_generations: int = 20, pop_size: int = 10, n_children: int = 5
            ) -> list[SearchResultV3]:
        self._log(f"Search v3 (rules as programs): {n_generations} gens, "
    ...
    def _eval_and_archive(self, rulesets: list[RuleSetV3], gen: int):
        for i, rs in enumerate(rulesets):
            ...
            t0 = time.time()
            m = evaluate_v3(rs, self.eval_cfg)
            elapsed = time.time() - t0
            sr = SearchResultV3(rs, m, gen=gen)
            self.all_results.append(sr)
            self._update_archive(sr)
            self._log(f"    {m.summary()}  ({elapsed:.1f}s)")

    def _update_archive(self, new: SearchResultV3):
    ...
    def top_results(self, n: int = 10) -> list[SearchResultV3]:
        return sorted(self.archive, key=lambda r: r.richness(), reverse=True)[:n]

    def report(self, n: int = 15) -> str:
        lines = [f"=== Search V3 Results — Rules as Programs (top {n}) ==="]
